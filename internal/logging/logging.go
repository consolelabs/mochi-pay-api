...
func ConfigureLogging(cfg config.View) {
	logrus.SetFormatter(newFormatter(cfg.GetString("LOGGING_FORMAT")))
	level := toLevel(cfg.GetString("LOGGING_LEVEL"))
	logrus.SetLevel(level)
	if isDebugLevel(level) {
		logrus.Warn("Trace logging level configured. Not recommended for production!")
	}
	logrus.Debugf("Configuring logging to %s", cfg.GetString("LOGGING_LEVEL"))
}
